...
func TestConvertMessages_DefaultFormat(t *testing.T) {
	...
	// Empty format should default to Luminox
	...
	_, ok := result.([]LuminoxMessage)
	assert.True(t, ok, "Default format should be Luminox")
}
...
func TestGetConvertedMessagesOutput(t *testing.T) {
	...
	// Luminox format should include public_urls
	assert.NotNil(t, result["public_urls"])
}

func TestGetConvertedMessagesOutput_NonLuminoxFormat(t *testing.T) {
	messages := []model.Message{
		createTestMessage("user", []model.Part{
			{Type: "text", Text: "Test"},
		}, nil),
	...
	// Non-Luminox formats should NOT include public_urls
...
func TestGetConvertedMessagesOutput_WithPublicURLs(t *testing.T) {
	...
	assert.True(t, hasURLs, "public_urls should exist for Luminox format")
