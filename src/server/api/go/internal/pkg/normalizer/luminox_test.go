...
func TestLuminoxNormalizer_NormalizeFromLuminoxMessage(t *testing.T) {
	normalizer := &LuminoxNormalizer{}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			role, parts, messageMeta, err := normalizer.NormalizeFromLuminoxMessage(json.RawMessage(tt.input))
			...
			if tt.wantErr {
				...
				assert.Equal(t, "luminox", messageMeta["source_format"])
			}
		})
	}
}

func TestLuminoxNormalizer_ValidatePartTypes(t *testing.T) {
	normalizer := &LuminoxNormalizer{}
...
func TestLuminoxNormalizer_MessageWithMeta(t *testing.T) {
	normalizer := &LuminoxNormalizer{}
	...
	role, parts, messageMeta, err := normalizer.NormalizeFromLuminoxMessage(json.RawMessage(input))
