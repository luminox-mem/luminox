...
import (
	...
	"github.com/memodb-io/Luminox/internal/modules/repo"
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(repo *MockUserRepo) {
				...
				repo.On("GetOrCreate", ctx, projectID, "alice@luminox.io").Return(expectedUser, nil)
			},
			wantErr: false,
		},
		{
			name:       "successful create new user",
			identifier: "newuser@luminox.io",
			setup: func(repo *MockUserRepo) {
				expectedUser := &model.User{
					ID:         uuid.New(),
					ProjectID:  projectID,
				...
				repo.On("GetOrCreate", ctx, projectID, "newuser@luminox.io").Return(expectedUser, nil)
			},
			wantErr: false,
		},
		{
		...
		},
		{
			...
			identifier: "alice@luminox.io",
			setup: func(repo *MockUserRepo) {
				repo.On("GetOrCreate", ctx, projectID, "alice@luminox.io").Return(nil, errors.New("database error"))
			},
			wantErr: true,
		},
	}
...
func TestUserService_Delete(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			identifier: "alice@luminox.io",
			setup: func(repo *MockUserRepo) {
				repo.On("Delete", ctx, projectID, "alice@luminox.io").Return(nil)
			},
			wantErr: false,
		},
		{
		...
		},
		{
			...
			identifier: "alice@luminox.io",
			setup: func(repo *MockUserRepo) {
				repo.On("Delete", ctx, projectID, "alice@luminox.io").Return(errors.New("deletion failed"))
			},
			wantErr: true,
		},
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(mockRepo *MockUserRepo) {
				...
				mockRepo.On("GetByIdentifier", ctx, projectID, "alice@luminox.io").Return(expectedUser, nil)
		...
		},
		{
			...
			setup: func(mockRepo *MockUserRepo) {
				...
				mockRepo.On("GetByIdentifier", ctx, projectID, "newuser@luminox.io").Return(expectedUser, nil)
