"""
Structured end-to-end exercise for the Luminox Python SDK.

The script drives every public client method so it can double as a
lightweight 2e2 test when pointed at a running Luminox instance.
"""
...
from luminox.errors import APIError, LuminoxError, TransportError
...
def resolve_credentials() -> tuple[str, str]:
    api_key = os.getenv("LUMINOX_API_KEY", "sk-ac-your-root-api-bearer-token")
    base_url = os.getenv("LUMINOX_BASE_URL", "http://localhost:8029/api/v1")
    return api_key, base_url


def exercise_spaces(client: LuminoxClient) -> tuple[str, dict[str, Any]]:
...
# def exercise_blocks(client: LuminoxClient, space_id: str) -> dict[str, Any]:
#     summary: dict[str, Any] = {}
...
def exercise_sessions(client: LuminoxClient, space_id: str) -> dict[str, Any]:
    summary: dict[str, Any] = {}
    ...
    # store message in luminox format
    luminox_blob = build_luminox_message(
        role="user",
        parts=["Hello from the SDK e2e test!"],
    )
    client.sessions.store_message(session_id, blob=luminox_blob, format="luminox")

    # store message in luminox format with file upload
    file_field = "retro_notes"
    file_blob = build_luminox_message(
        role="user",
    ...
    tool_blob = build_luminox_message(
        role="assistant",
        parts=[
    ...
    client.sessions.store_message(session_id, blob=tool_blob, format="luminox")
...
def exercise_disks(client: LuminoxClient) -> dict[str, Any]:
    summary: dict[str, Any] = {}
...
def run() -> dict[str, Any]:
    ...
    with LuminoxClient(api_key=api_key, base_url=base_url) as client:
