...
This script is meant to be run against a live Luminox API instance.
"""
...
def resolve_credentials() -> tuple[str, str]:
    api_key = (
        os.getenv("LUMINOX_API_KEY")
        or os.getenv("API_KEY")
        or "sk-ac-your-root-api-bearer-token"
    )
    base_url = (
        os.getenv("LUMINOX_BASE_URL") or os.getenv("BASE_URL") or "http://localhost:8029/api/v1"
    )
...
def store_text(client: LuminoxClient, session_id: str, text: str) -> None:
...
def exercise_basic_middle_out(client: LuminoxClient) -> None:
...
def exercise_even_determinism(client: LuminoxClient) -> None:
...
def exercise_keep_tail(client: LuminoxClient) -> None:
...
def exercise_tool_pairing(client: LuminoxClient) -> None:
...
def exercise_validation_errors(client: LuminoxClient) -> None:
...
def main() -> None:
    ...
    client = LuminoxClient(api_key=api_key, base_url=base_url)
