"""
Example usage of the Luminox space search APIs.
...
from luminox.errors import APIError


def resolve_credentials() -> tuple[str, str]:
    """Get API credentials from environment variables."""
    api_key = os.getenv("LUMINOX_API_KEY", "sk-proj-your-project-api-key")
    base_url = os.getenv("LUMINOX_BASE_URL", "http://localhost:8029/api/v1")
    return api_key, base_url


def example_experience_search(client: LuminoxClient, space_id: str) -> None:
    """
...
async def example_async_search() -> None:
    ...
    """
    from luminox import AsyncLuminoxClient

    api_key, base_url = resolve_credentials()

    async with AsyncLuminoxClient(api_key=api_key, base_url=base_url) as client:
        # Get first space
