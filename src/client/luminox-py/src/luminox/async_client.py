"""
High-level asynchronous client for the Luminox API.
"""
...
class LuminoxAsyncClient:
    ...
    def __init__(
        ...
        *,
        ...
        """
        Initialize the Luminox async client.

        Args:
            api_key: API key for authentication. Can also be set via LUMINOX_API_KEY env var.
            base_url: Base URL for the API. Defaults to DEFAULT_BASE_URL. Can also be set via LUMINOX_BASE_URL env var.
            timeout: Request timeout in seconds. Defaults to 10.0. Can also be set via LUMINOX_TIMEOUT env var.
                   Can also be an httpx.Timeout object.
            user_agent: Custom user agent string. Can also be set via LUMINOX_USER_AGENT env var.
        ...
        api_key = api_key or os.getenv("LUMINOX_API_KEY")
        if not api_key or not api_key.strip():
            raise ValueError(
                "api_key is required. Provide it either as a parameter (api_key='...') "
                "or set the LUMINOX_API_KEY environment variable."
            )

        # Load other parameters from environment variables if not provided
        if base_url is None:
            base_url = os.getenv("LUMINOX_BASE_URL", DEFAULT_BASE_URL)
        base_url = base_url.rstrip("/")

        if user_agent is None:
            user_agent = os.getenv("LUMINOX_USER_AGENT", DEFAULT_USER_AGENT)

        # Handle timeout: support both float and httpx.Timeout
        if timeout is None:
            timeout_str = os.getenv("LUMINOX_TIMEOUT")
            if timeout_str:
                try:
    ...
    async def __aenter__(self) -> "LuminoxAsyncClient":
        return self
