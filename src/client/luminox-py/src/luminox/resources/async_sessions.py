...
MessageBlob = LuminoxMessage | ChatCompletionMessageParam | MessageParam
...
class AsyncSessionsAPI:
    ...
    async def store_message(
        ...
        *,
        blob: MessageBlob,
        format: Literal["luminox", "openai", "anthropic", "gemini"] = "openai",
        ...
        Args:
            session_id: The UUID of the session.
            blob: The message blob in Luminox, OpenAI, Anthropic, or Gemini format.
            format: The format of the message blob. Defaults to "openai".
            file_field: The field name for file upload. Only used when format is "luminox".
                Required if file is provided. Defaults to None.
            file: Optional file upload. Only used when format is "luminox". Defaults to None.

        Returns:
            The created Message object.

        Raises:
            ValueError: If format is invalid, file/file_field provided for non-luminox format,
                or file is provided without file_field for luminox format.
        """
        if format not in {"luminox", "openai", "anthropic", "gemini"}:
            raise ValueError(
                "format must be one of {'luminox', 'openai', 'anthropic', 'gemini'}"
            )

        # File upload is only supported for luminox format
        if format != "luminox" and (file is not None or file_field is not None):
            raise ValueError(
                "file and file_field parameters are only supported when format is 'luminox'"
            )
        ...
        if format == "luminox":
            if isinstance(blob, Mapping):
                payload["blob"] = blob
            elif isinstance(blob, LuminoxMessage):
                payload["blob"] = asdict(blob)
            else:
                raise ValueError(
                    f"Invalid blob type: {type(blob)} when format is 'luminox'. Expected Mapping or LuminoxMessage"
                )

            # Handle file upload for luminox format
    ...
    async def get_messages(
        ...
        *,
        ...
        Args:
            ...
            format: The format of the messages. Defaults to "openai". Supports "luminox", "openai", "anthropic", or "gemini".
            time_desc: Order by created_at descending if True, ascending if False. Defaults to None.
