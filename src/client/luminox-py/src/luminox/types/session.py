...
class TaskData(BaseModel):
    ...
    This schema matches the TaskData model in luminox_core/schema/session/task.py
...
class GetMessagesOutput(BaseModel):
    ...
    - format="luminox": items is list[Message] (Luminox format)
    ...
    public_urls: dict[str, PublicURL] | None = Field(
        None,
        description="Map of SHA256 hash to PublicURL (only included when format='luminox')",
    )
