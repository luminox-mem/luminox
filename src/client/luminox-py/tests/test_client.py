...
from luminox.client import LuminoxClient, FileUpload  # noqa: E402
from luminox.messages import build_luminox_message  # noqa: E402
from luminox.errors import APIError, TransportError  # noqa: E402
...
@pytest.fixture
def client() -> LuminoxClient:
    client = LuminoxClient(api_key="token")
    try:
        yield client
    finally:
        client.close()
...
def test_build_luminox_message_with_meta() -> None:
    message = build_luminox_message(
        role="assistant",
        parts=["hi"],
        meta={"name": "bot"},
    )
...
def test_handle_response_returns_data() -> None:
    ...
    data = LuminoxClient._handle_response(resp, unwrap=True)
    assert data == {"ok": True}
...
def test_handle_response_app_code_error() -> None:
    ...
    with pytest.raises(APIError) as ctx:
        LuminoxClient._handle_response(resp, unwrap=True)
    assert ctx.value.code == 500
    assert ctx.value.status_code == 200


@patch("luminox.client.httpx.Client.request")
def test_request_transport_error(mock_request) -> None:
    ...
    with LuminoxClient(api_key="token") as client:
        with pytest.raises(TransportError):
            client.spaces.list()


@patch("luminox.client.LuminoxClient.request")
def test_ping_returns_pong(mock_request, client: LuminoxClient) -> None:
...
def test_store_message_with_files_uses_multipart_payload(
    mock_request, client: LuminoxClient
) -> None:
...
@patch("luminox.client.LuminoxClient.request")
def test_store_message_accepts_luminox_message(
    mock_request, client: LuminoxClient
) -> None:
    mock_request.return_value = {
        "id": "msg-id",
    ...
    client.sessions.store_message("session-id", blob=blob, format="luminox")
...
@patch("luminox.client.LuminoxClient.request")
def test_store_message_rejects_file_for_non_luminox_format(
    mock_request, client: LuminoxClient
) -> None:
    class _DummyStream:
        def read(self) -> bytes:
...
@patch("luminox.client.LuminoxClient.request")
def test_store_message_rejects_file_field_for_non_luminox_format(
    mock_request, client: LuminoxClient
) -> None:
    with pytest.raises(
        ValueError,
...
@patch("luminox.client.LuminoxClient.request")
def test_sessions_get_tasks_with_filters(mock_request, client: LuminoxClient) -> None:
    mock_request.return_value = {"items": [], "ids": [], "has_more": False}
...
@patch("luminox.client.LuminoxClient.request")
def test_sessions_get_learning_status(mock_request, client: LuminoxClient) -> None:
    mock_request.return_value = {
...
@patch("luminox.client.LuminoxClient.request")
def test_sessions_get_token_counts(mock_request, client: LuminoxClient) -> None:
    mock_request.return_value = {
        "total_tokens": 1234,
    }
...
@patch("luminox.client.LuminoxClient.request")
def test_blocks_list_without_filters(mock_request, client: LuminoxClient) -> None:
    mock_request.return_value = []
...
@patch("luminox.client.LuminoxClient.request")
def test_blocks_list_with_filters(mock_request, client: LuminoxClient) -> None:
    mock_request.return_value = []
...
# @patch("luminox.client.LuminoxClient.request")
# def test_blocks_create_root_payload(mock_request, client: LuminoxClient) -> None:
#     mock_request.return_value = {
#         "id": "block",
#         "space_id": "space-id",
#         "type": "folder",
...
@patch("luminox.client.LuminoxClient.request")
def test_blocks_move_requires_payload(mock_request, client: LuminoxClient) -> None:
...
@patch("luminox.client.LuminoxClient.request")
def test_blocks_move_with_parent(mock_request, client: LuminoxClient) -> None:
...
@patch("luminox.client.LuminoxClient.request")
def test_blocks_move_with_sort(mock_request, client: LuminoxClient) -> None:
    mock_request.return_value = {"status": "ok"}
...
@patch("luminox.client.LuminoxClient.request")
def test_disks_create_hits_disk_endpoint(mock_request, client: LuminoxClient) -> None:
    mock_request.return_value = {
        "id": "disk",
...
def test_artifacts_aliases_disk_artifacts(client: LuminoxClient) -> None:
...
@patch("luminox.client.LuminoxClient.request")
def test_skills_delete_hits_skills_endpoint(mock_request, client: LuminoxClient) -> None:
...
@patch("luminox.client.LuminoxClient.request")
def test_users_list_without_filters(mock_request, client: LuminoxClient) -> None:
    mock_request.return_value = {
        "items": [
            {
                "id": "123e4567-e89b-12d3-a456-426614174000",
...
@patch("luminox.client.LuminoxClient.request")
def test_users_list_with_filters(mock_request, client: LuminoxClient) -> None:
    mock_request.return_value = {
        "items": [
            {
                "id": "123e4567-e89b-12d3-a456-426614174000",
...
@patch("luminox.client.LuminoxClient.request")
def test_users_get_resources(mock_request, client: LuminoxClient) -> None:
    mock_request.return_value = {
        "counts": {
            "spaces_count": 5,
            "sessions_count": 10,
    ...
    result = client.users.get_resources("alice@luminox.io")
...
@patch("luminox.client.LuminoxClient.request")
def test_users_delete(mock_request, client: LuminoxClient) -> None:
    mock_request.return_value = None

    client.users.delete("alice@luminox.io")
