...
export const LuminoxMessageSchema = z.object({
  role: z.enum(['user', 'assistant']),
  parts: z.array(MessagePartSchema),
  meta: z.record(z.string(), z.unknown()).nullable().optional(),
});

export type LuminoxMessageInput = z.infer<typeof LuminoxMessageSchema>;

export class LuminoxMessage {
  role: 'user' | 'assistant';
  parts: MessagePart[];
  meta?: Record<string, unknown> | null;

  constructor(data: LuminoxMessageInput) {
    this.role = data.role;
...
export function buildLuminoxMessage(options: {
  role: 'user' | 'assistant';
