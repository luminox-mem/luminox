...
export interface LuminoxClientOptions {
  apiKey?: string | null;
  baseUrl?: string | null;
  timeout?: number | null;
...
export class LuminoxClient implements RequesterProtocol {
  ...
  constructor(options: LuminoxClientOptions = {}) {
    ...
    this._baseUrl =
      options.baseUrl ||
      (typeof process !== 'undefined' && process.env?.LUMINOX_BASE_URL) ||
      DEFAULT_BASE_URL;
    this._baseUrl = this._baseUrl.replace(/\/$/, '');

    this._userAgent =
      options.userAgent ||
      (typeof process !== 'undefined' && process.env?.LUMINOX_USER_AGENT) ||
      DEFAULT_USER_AGENT;

    this._timeout =
      options.timeout ??
      (typeof process !== 'undefined' && process.env?.LUMINOX_TIMEOUT
        ? parseFloat(process.env.LUMINOX_TIMEOUT)
        : 320000);
