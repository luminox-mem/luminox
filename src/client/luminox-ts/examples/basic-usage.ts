...
/**
 * Structured end-to-end exercise for the Luminox TypeScript SDK.
 *
 * The script drives every public client method so it can double as a
 * lightweight e2e test when pointed at a running Luminox instance.
 */
...
function resolveCredentials(): { apiKey: string; baseUrl: string } {
  const apiKey = process.env.LUMINOX_API_KEY || 'sk-ac-your-root-api-bearer-token';
  const baseUrl = process.env.LUMINOX_BASE_URL || 'http://localhost:8029/api/v1';
...
async function exerciseSessions(
  ...
  const luminoxBlob = buildLuminoxMessage({
    role: 'user',
    parts: ['Hello from the SDK e2e test!'],
  });
  await client.sessions.storeMessage(sessionId, luminoxBlob, { format: 'luminox' });

  // Store message in luminox format with file upload
  const fileField = 'retro_notes';
  const fileBlob = buildLuminoxMessage({
    role: 'user',
  ...
  const toolBlob = buildLuminoxMessage({
    role: 'assistant',
    parts: [
  ...
  await client.sessions.storeMessage(sessionId, toolBlob, { format: 'luminox' });
...
async function exerciseDisks(client: LuminoxClient): Promise<Record<string, unknown>> {
  const summary: Record<string, unknown> = {};
...
async function run(): Promise<Record<string, unknown>> {
  ...
  const client = new LuminoxClient({ apiKey, baseUrl });
...
async function main(): Promise<void> {
  try {
    ...
    if (error instanceof LuminoxError) {
