/**
 * Integration tests for the Luminox TypeScript SDK.
 * These tests require a running Luminox API instance.
 */

import { LuminoxClient, MessagePart, FileUpload, buildLuminoxMessage } from '../src/index';

describe('LuminoxClient Integration Tests', () => {
  ...
  beforeAll(() => {
    client = new LuminoxClient({ apiKey, baseUrl });
  });
  ...
  describe('Sessions API', () => {
    ...
    test('should store message with file upload', async () => {
      ...
      const blob = buildLuminoxMessage({
        role: 'user',
  ...
  describe('Users API', () => {
    ...
    beforeAll(() => {
      testUserIdentifier = `test-user-${Date.now()}@luminox.io`;
    });
