...
import (
	...
	"github.com/memodb-io/Luminox/luminox-cli/internal/docker"
...
var DockerCmd = &cobra.Command{
	...
	Long: `Manage Docker Compose services for Luminox projects.
...
func runDockerUp(cmd *cobra.Command, args []string) error {
	...
	if detachedMode {
		...
		if err := docker.WaitForHealth(projectDir, composeFile, 120*time.Second); err != nil {
			...
			fmt.Println("   Services may still be starting. Check status with: luminox docker status")
		} else {
			fmt.Println()
...
func promptEnvConfig() (*docker.EnvConfig, error) {
	...
	if err := survey.AskOne(&survey.Input{
		Message: "4. Pass a string to build Luminox token:",
	...
	fmt.Printf("  🔑 \033[1m\033[93mLUMINOX_API_KEY=\"%s\"\033[0m\n", completeAPIKey)

	// Prompt for Core Config YAML File (optional)
	var coreConfigYAMLFile string
	if err := survey.AskOne(&survey.Input{
		Message: "5. Configure Luminox by passing an existing config.yaml (optional):",
		Default: "./config.yaml",
