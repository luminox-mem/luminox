...
import (
	...
	"github.com/memodb-io/Luminox/luminox-cli/internal/config"
	"github.com/memodb-io/Luminox/luminox-cli/internal/git"
	"github.com/memodb-io/Luminox/luminox-cli/internal/template"
...
var CreateCmd = &cobra.Command{
	...
	Short: "Create a new Luminox project",
	Long: `Create a new Luminox project with a template.
...
Use --template-path to specify a custom template folder from:
  https://github.com/memodb-io/Luminox-Examples
...
func init() {
	CreateCmd.Flags().StringVarP(&templatePath, "template-path", "t", "", "Custom template folder path from Luminox-Examples repository (e.g., python/custom-template)")
}
...
func runCreate(cmd *cobra.Command, args []string) error {
	...
	if len(args) > 0 {
		...
		defaultName := "my-luminox-app"
		prompt := &survey.Input{
			Message: "Project name:",
			Help:    "Enter a name for your project (e.g., my-luminox-app)",
			Default: defaultName,
		}
	...
	if templatePath != "" {
		...
		templateConfig = &template.Config{
			Repo:        "https://github.com/memodb-io/Luminox-Examples",
			Path:        templatePath,
