...
var UpgradeCmd = &cobra.Command{
	Use:   "upgrade",
	Short: "Upgrade Luminox CLI to the latest version",
	Long: `Upgrade Luminox CLI to the latest version.

This command downloads and installs the latest version of Luminox CLI
by executing the installation script from install.luminox.io.
...
func runUpgrade(cmd *cobra.Command, args []string) error {
	...
	installScriptURL := "https://install.luminox.io"
	...
	fmt.Printf("   Run 'luminox version' to verify the new version\n")
...
func getCurrentVersionFallback() string {
	...
	versionCmd := exec.Command("luminox", "version")
	output, err := versionCmd.Output()
	if err != nil {
		return "unknown"
	}

	// Parse output: "Luminox CLI version v0.0.1"
