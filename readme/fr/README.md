...
> Une mauvaise conception de schéma rend vos données les plus précieuses coûteuses et lentes. Luminox gère le stockage et la récupération du contexte via PG, Redis et S3.
>
...
> Vous connaissez l'ingénierie de contexte, mais vous l'écrivez toujours de zéro. Luminox est livré avec des méthodes d'édition de contexte intégrées et un todo agent prêt à l'emploi.
>
> Gérer l'état de l'agent ? Un jeu d'enfant.

**👀 Problème 3 : Vous ne pouvez pas voir comment votre agent se comporte.** 

> Vos utilisateurs sont-ils vraiment satisfaits ? Luminox suit les tâches par session et vous montre le taux de réussite réel de votre agent.
>
> Arrêtez de vous obséder sur les coûts de tokens, améliorez d'abord l'agent.

**🧠 Problème 4 : Votre agent est imprévisible.**

> Peut-il apprendre de ses succès ? L'experience agent d'Luminox se souvient des exécutions réussies et les transforme en SOPs d'utilisation d'outils réutilisables.
>
> La cohérence est tout.



Pour résoudre ces problèmes d'un coup, Luminox devient la **Plateforme de Données de Contexte** :
...
- **Context Engineering**
  - [Session](https://docs.luminox.io/store/messages/multi-provider) : stockage de messages unifié pour n'importe quel LLM, n'importe quelle modalité.
  - [Disk](https://docs.luminox.io/store/disk) : sauvegarder/télécharger des artifacts avec chemin de fichier.
  - [Context Editing](https://docs.luminox.io/store/editing) - gérez votre fenêtre de contexte en une API.

<div align="center">
    <picture>
      <img alt="Luminox Learning" src="../../assets/luminox-context-engineering.png" width="80%">
    </picture>
  <p>Context Engineering dans Luminox</p>
</div>

- **Observer les tâches de l'agent et les retours utilisateurs**
  - [Task](https://docs.luminox.io/observe/agent_tasks) : collecter l'état de travail de l'agent, le progrès et les préférences en temps quasi réel.
- **Auto-apprentissage de l'agent**
  - [Experience](https://docs.luminox.io/learn/advance/experience-agent) : laisser l'agent apprendre des SOPs pour chaque utilisateur.
- **Voir tout dans un [tableau de bord](https://docs.luminox.io/observe/dashboard)**
...
# 🚀 Se Connecter à Luminox

1. Allez sur [Luminox.io](https://luminox.io), réclamez vos crédits gratuits.
...
<details>
<summary>💻 Auto-héberger Luminox</summary>

Nous avons un `luminox-cli` pour vous aider à faire une preuve de concept rapide. Téléchargez-le d'abord dans votre terminal :

```bash
curl -fsSL https://install.luminox.io | sh
```

Vous devriez avoir [docker](https://www.docker.com/get-started/) installé et une OpenAI API Key pour démarrer un backend Luminox sur votre ordinateur :
...
> [!IMPORTANT]
>
> Assurez-vous que votre LLM a la capacité d'[appeler des outils](https://platform.openai.com/docs/guides/function-calling). Par défaut, Luminox utilisera `gpt-4.1`.

`luminox docker up` créera/utilisera `.env` et `config.yaml` pour Luminox, et créera un dossier `db` pour persister les données.
...
# 🧐 Utiliser Luminox pour construire un Agent

Téléchargez des scripts de bout en bout avec `luminox` :
...
> [!NOTE]
>
> Consultez notre dépôt d'exemples pour plus de modèles : [Luminox-Examples](https://github.com/memodb-io/Luminox-Examples).
>
> Nous préparons plus d'applications Agent full-stack ! [Dites-nous ce que vous voulez !](https://discord.luminox.io)
...
Nous maintenons les SDKs Python [![pypi](https://img.shields.io/pypi/v/luminox.svg)](https://pypi.org/project/luminox/) et Typescript [![npm](https://img.shields.io/npm/v/luminox.svg?logo=npm&logoColor=fff&style=flat&labelColor=2C2C2C&color=28CF8D)](https://www.npmjs.com/package/luminox). Les extraits de code ci-dessous utilisent Python.
...
# Si vous utilisez Luminox auto-hébergé :
# client = LuminoxClient(
...
Luminox peut gérer les sessions d'agents et les artifacts.

### Enregistrer les Messages [📖](https://docs.luminox.io/api-reference/session/store-message-to-session)

Luminox offre un stockage persistant pour les données de messages. Lorsque vous appelez `session.store_message`, Luminox persistera le message et commencera à surveiller cette session :
...
> [📖](https://docs.luminox.io/store/messages/multi-modal) Nous prenons également en charge le stockage de messages multi-modaux et le SDK anthropic.


</details>

### Charger les Messages [📖](https://docs.luminox.io/api-reference/session/get-messages-from-session)
...
## Observer [📖](https://docs.luminox.io/observe)

Pour chaque session, Luminox lancera **automatiquement** un agent en arrière-plan pour suivre la progression des tâches et les retours des utilisateurs. **C'est comme un agent TODO en arrière-plan**. Luminox l'utilisera pour observer votre taux de réussite quotidien des agents.
...
> Vous n'avez pas besoin de l'appeler en production, Luminox a un [mécanisme de tampon](https://docs.luminox.io/observe/buffer) pour s'assurer que l'extraction de la tâche est terminée au bon moment.
...
Luminox peut collecter un tas de sessions et apprendre des compétences (SOPs) sur la façon d'appeler des outils pour certaines tâches.

### Apprendre des Compétences dans un `Space` [📖](https://docs.luminox.io/learn/skill-space)
...
Ce qu'Luminox fera en arrière-plan :
...
### Rechercher des Compétences depuis un `Space` [📖](https://docs.luminox.io/learn/search-skills)
...
Luminox prend en charge les modes `fast` et `agentic` pour la recherche. Le premier utilise des embeddings pour faire correspondre les compétences. Le second utilise un Experience Agent pour explorer tout le `Space` et tente de couvrir chaque compétence nécessaire.
...
Pour mieux comprendre ce qu'Luminox peut faire, veuillez consulter [notre documentation](https://docs.luminox.io/)



# ❤️ Restez à Jour

Marquez Luminox d'une étoile sur Github pour soutenir et recevoir des notifications instantanées 
...
-   [Discutez avec les Constructeurs sur Luminox Discord](https://discord.luminox.io) 👻 
-  [Suivez Luminox sur X](https://x.com/luminox_io) 𝕏 
