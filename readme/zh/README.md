...
> 糟糕的架构设计使你最有价值的数据变得昂贵且缓慢。Luminox通过PG、Redis和S3处理上下文存储和检索。
>
> ChatGPT、Gemini、Anthropic、图片、音频、文件...我们都支持。

**⏰ 问题2：长时间运行的Agent是个噩梦。** 

> 你了解上下文工程，但你总是从头开始写。Luminox内置了上下文编辑方法和开箱即用的todo agent。
>
> 管理Agent状态？小菜一碟。

**👀 问题3：你无法看到你的Agent表现如何。** 

> 你的用户真的满意吗？Luminox跟踪每个会话的任务，并向你展示Agent的实际成功率。
>
> 不要只关注token成本，先改进Agent。

**🧠 问题4：你的Agent时好时坏。**

> 它能从成功中学习吗？Luminox的experience agent记住成功的运行，并将它们转化为可重用的工具使用SOP。
>
> 一致性就是一切。



为了一次解决这些问题，Luminox成为了**上下文数据平台**：
...
- **Context Engineering**
  - [Session](https://docs.luminox.io/store/messages/multi-provider): 为任何LLM、任何模态提供统一的消息存储。
  - [Disk](https://docs.luminox.io/store/disk): 使用文件路径保存/下载artifacts。
  - [Context Editing](https://docs.luminox.io/store/editing) - 一个API管理你的上下文窗口。

<div align="center">
    <picture>
      <img alt="Luminox Learning" src="../../assets/luminox-context-engineering.png" width="80%">
    </picture>
  <p>Luminox中的Context Engineering</p>
</div>

- **观察Agent任务和用户反馈**
  - [Task](https://docs.luminox.io/observe/agent_tasks): 近实时收集Agent的工作状态、进度和偏好。
- **Agent自我学习**
  - [Experience](https://docs.luminox.io/learn/advance/experience-agent): 让Agent为每个用户学习SOP。
- **在一个[仪表板](https://docs.luminox.io/observe/dashboard)中查看所有内容**
...
1. 前往 [Luminox.io](https://luminox.io)，领取免费额度。
...
<details>
<summary>💻 自托管Luminox</summary>

我们有一个 `luminox-cli` 来帮助你快速进行概念验证。首先在终端中下载它：

```bash
curl -fsSL https://install.luminox.io | sh
```

你应该安装 [docker](https://www.docker.com/get-started/) 并拥有 OpenAI API Key，以便在计算机上启动 Luminox 后端：
...
> [!IMPORTANT]
>
> 确保你的LLM有[调用工具](https://platform.openai.com/docs/guides/function-calling)的能力。默认情况下，Luminox将使用 `gpt-4.1`。

`luminox docker up` 将为Luminox创建/使用 `.env` 和 `config.yaml`，并创建 `db` 文件夹来持久化数据。
...
> [!NOTE]
>
> 查看我们的示例仓库获取更多模板：[Luminox-Examples](https://github.com/memodb-io/Luminox-Examples)。
>
> 我们正在开发更多全栈Agent应用！[告诉我们你想要什么！](https://discord.luminox.io)
...
我们维护 Python [![pypi](https://img.shields.io/pypi/v/luminox.svg)](https://pypi.org/project/luminox/) 和 Typescript [![npm](https://img.shields.io/npm/v/luminox.svg?logo=npm&logoColor=fff&style=flat&labelColor=2C2C2C&color=28CF8D)](https://www.npmjs.com/package/luminox) SDK。下面的代码片段使用Python。
...
> [📖 异步客户端文档](https://docs.luminox.io/settings/core)



## 存储

Luminox可以管理Agent会话和Artifacts。

### 保存消息 [📖](https://docs.luminox.io/api-reference/session/store-message-to-session)

Luminox为消息数据提供持久化存储。当你调用 `session.store_message` 时，Luminox将持久化消息并开始监控此会话：
...
> [📖](https://docs.luminox.io/store/messages/multi-modal) 我们还支持多模态消息存储和anthropic SDK。


</details>

### 加载消息 [📖](https://docs.luminox.io/api-reference/session/get-messages-from-session)
...
## 观察 [📖](https://docs.luminox.io/observe)

对于每个会话，Luminox将**自动**启动一个后台Agent来跟踪任务进度和用户反馈。**它就像一个后台TODO Agent**。Luminox将使用它来观察你日常Agent的成功率。
...
> 你不需要在生产环境中调用它，Luminox有一个[缓冲机制](https://docs.luminox.io/observe/buffer)来确保任务提取在正确的时间完成。
...
Luminox可以收集大量会话，并学习如何为某些任务调用工具的技能（SOP）。

### 将技能学习到 `Space` [📖](https://docs.luminox.io/learn/skill-space)
...
### 从 `Space` 搜索技能 [📖](https://docs.luminox.io/learn/search-skills)
...
Luminox支持 `fast` 和 `agentic` 搜索模式。前者使用嵌入来匹配技能。后者使用Experience Agent探索整个 `Space`，并尝试涵盖所需的每个技能。
...
要更好地了解Luminox的功能，请查看 [我们的文档](https://docs.luminox.io/)



# ❤️ 保持更新

在Github上为Luminox加星标以支持并接收即时通知 
...
-   [在Luminox Discord上与构建者讨论](https://discord.luminox.io) 👻 
-  [在X上关注Luminox](https://x.com/luminox_io) 𝕏 
