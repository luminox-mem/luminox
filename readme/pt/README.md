...
> Design de schema ruim torna seus dados mais valiosos caros e lentos. Luminox lida com armazenamento e recuperação de contexto via PG, Redis e S3.
>
> ChatGPT, Gemini, Anthropic, imagens, áudio, arquivos... temos você coberto.

**⏰ Problema 2: Agents de longa duração são um pesadelo.** 

> Você conhece engenharia de contexto, mas está sempre escrevendo do zero. Luminox vem com métodos de edição de contexto integrados e um todo agent pronto para usar.
>
> Gerenciar estado do agent? Moleza.

**👀 Problema 3: Você não consegue ver como seu agent está se saindo.** 

> Seus usuários estão realmente satisfeitos? Luminox rastreia tarefas por sessão e mostra a taxa de sucesso real do seu agent.
>
> Pare de se obcecar com custos de tokens, melhore o agent primeiro.

**🧠 Problema 4: Seu agent é imprevisível.**

> Ele pode aprender com seus sucessos? O experience agent do Luminox lembra execuções bem-sucedidas e as transforma em SOPs de uso de ferramentas reutilizáveis.
>
> Consistência é tudo.



Para resolver esses problemas de uma vez, Luminox se torna a **Plataforma de Dados de Contexto**:
...
- **Context Engineering**
  - [Session](https://docs.luminox.io/store/messages/multi-provider): armazenamento de mensagens unificado para qualquer LLM, qualquer modalidade.
  - [Disk](https://docs.luminox.io/store/disk): salvar/baixar artifacts com caminho de arquivo.
  - [Context Editing](https://docs.luminox.io/store/editing) - gerencie sua janela de contexto em uma API.

<div align="center">
    <picture>
      <img alt="Luminox Learning" src="../../assets/luminox-context-engineering.png" width="80%">
    </picture>
  <p>Context Engineering no Luminox</p>
</div>

- **Observar tarefas do agent e feedback do usuário**
  - [Task](https://docs.luminox.io/observe/agent_tasks): coletar status de trabalho do agent, progresso e preferências em tempo quase real.
- **Auto-aprendizado do agent**
  - [Experience](https://docs.luminox.io/learn/advance/experience-agent): deixe o agent aprender SOPs para cada usuário.
- **Visualizar tudo em um [dashboard](https://docs.luminox.io/observe/dashboard)**
...
1. Vá para [Luminox.io](https://luminox.io), resgate seus créditos grátis.
...
<details>
<summary>💻 Auto-hospedar Luminox</summary>

Temos um `luminox-cli` para ajudá-lo a fazer uma prova de conceito rápida. Baixe-o primeiro no seu terminal:

```bash
curl -fsSL https://install.luminox.io | sh
```

Você deve ter [docker](https://www.docker.com/get-started/) instalado e uma OpenAI API Key para iniciar um backend Luminox no seu computador:
...
> [!IMPORTANT]
>
> Certifique-se de que seu LLM tenha a capacidade de [chamar ferramentas](https://platform.openai.com/docs/guides/function-calling). Por padrão, Luminox usará `gpt-4.1`.

`luminox docker up` criará/usará `.env` e `config.yaml` para Luminox, e criará uma pasta `db` para persistir dados.
...
Baixe scripts de ponta a ponta com `luminox`:
...
> [!NOTE]
>
> Confira nosso repositório de exemplos para mais templates: [Luminox-Examples](https://github.com/memodb-io/Luminox-Examples).
>
> Estamos preparando mais aplicações Agent full-stack! [Diga-nos o que você quer!](https://discord.luminox.io)
...
Mantemos SDKs Python [![pypi](https://img.shields.io/pypi/v/luminox.svg)](https://pypi.org/project/luminox/) e Typescript [![npm](https://img.shields.io/npm/v/luminox.svg?logo=npm&logoColor=fff&style=flat&labelColor=2C2C2C&color=28CF8D)](https://www.npmjs.com/package/luminox). Os trechos de código abaixo usam Python.
...
# Se você estiver usando Luminox auto-hospedado:
# client = LuminoxClient(
...
Luminox pode gerenciar sessões de agents e artifacts.

### Salvar Mensagens [📖](https://docs.luminox.io/api-reference/session/store-message-to-session)

Luminox oferece armazenamento persistente para dados de mensagens. Quando você chama `session.store_message`, o Luminox persistirá a mensagem e começará a monitorar esta sessão:
...
> [📖](https://docs.luminox.io/store/messages/multi-modal) Também suportamos armazenamento de mensagens multi-modal e SDK anthropic.


</details>

### Carregar Mensagens [📖](https://docs.luminox.io/api-reference/session/get-messages-from-session)
...
Para cada sessão, o Luminox **automaticamente** iniciará um agent em segundo plano para rastrear o progresso da tarefa e o feedback do usuário. **É como um agent TODO em segundo plano**. Luminox o usará para observar sua taxa de sucesso diária do agent.
...
> Você não precisa chamá-lo em produção, o Luminox tem um [mecanismo de buffer](https://docs.luminox.io/observe/buffer) para garantir que a extração da tarefa seja concluída no momento certo.
...
Luminox pode coletar várias sessões e aprender habilidades (SOPs) sobre como chamar ferramentas para certas tarefas.

### Aprender Habilidades em um `Space` [📖](https://docs.luminox.io/learn/skill-space)
...
O que o Luminox fará em segundo plano:
...
### Pesquisar Habilidades de um `Space` [📖](https://docs.luminox.io/learn/search-skills)
...
Luminox suporta modos `fast` e `agentic` para pesquisa. O primeiro usa embeddings para corresponder habilidades. O segundo usa um Experience Agent para explorar todo o `Space` e tenta cobrir cada habilidade necessária.
...
Para entender melhor o que o Luminox pode fazer, por favor, consulte [nossa documentação](https://docs.luminox.io/)



# ❤️ Mantenha-se Atualizado

Marque o Luminox com uma estrela no Github para apoiar e receber notificações instantâneas 
...
-   [Discuta com Construtores no Luminox Discord](https://discord.luminox.io) 👻 
-  [Siga o Luminox no X](https://x.com/luminox_io) 𝕏 
