...
Luminox provides pre-built skill tools that allow LLMs to access reusable knowledge bundles (skills) through function calling. Skills contain files, scripts, and instructions that help agents perform specialized tasks.
...
ctx = SKILL_TOOLS.format_context(luminox_client, skill_ids)
...
import { LuminoxClient, SKILL_TOOLS } from 'luminox-sdk';
...
const ctx = await SKILL_TOOLS.formatContext(luminoxClient, skillIds);
...
<Step title="Initialize clients and preload skills">
  Set up both the Luminox client and your LLM client (OpenAI or Anthropic). Preload skills by their UUIDs to create the context.
