...
Luminox provides a flexible message storage system that supports multiple LLM providers (OpenAI, Anthropic, Google Gemini). You can store messages then retrieve them later for analysis, debugging, or continuing conversations.
...
<Info>
Luminox provides built-in observability features to help you monitor and analyze your message sessions in real-time.
</Info>
...
Access the Luminox dashboard to visualize message flows, traces, and session analytics.
</Card>
...
Luminox supports three message formats for maximum compatibility:
...
<Tip>
You can store messages in one format and retrieve them in another. Luminox automatically handles format conversion.
</Tip>
