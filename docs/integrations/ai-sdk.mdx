---
title: "Vercel AI SDK"
description: "Integrate Vercel AI SDK with Luminox for session persistence, tool calling, and task extraction"
---

The Vercel AI SDK provides a unified interface for building AI applications with support for multiple providers. When integrated with Luminox, you get persistent session management, automatic task extraction, and seamless conversation resumption across sessions.
...
Use `luminox-cli` to quickly set up a Vercel AI SDK project with Luminox integration:

```bash
luminox create my-ai-project --template-path "typescript/vercel-ai-basic"
```
...
Install Vercel AI SDK and Luminox TypeScript packages:

```bash
npm install ai @ai-sdk/openai luminox dotenv zod
```

Or with yarn:
```bash
yarn add ai @ai-sdk/openai luminox dotenv zod
```
</Step>
...
Create Vercel AI SDK provider and Luminox client instances:
...
The Vercel AI SDK integration works by storing conversation messages to Luminox in OpenAI message format. The SDK uses `generateText` for text generation and requires manual tool execution.
...
4. **Store messages**: Append each message (user, assistant, and tool) to Luminox
...
<Warning>
**Message format limitations**: Vercel AI SDK v5 only accepts 'user' and 'assistant' roles in the messages array. Tool results must be converted to user messages for the next iteration, but marked as internal so they aren't sent to Luminox.
</Warning>
...
Here's the core pattern for integrating Vercel AI SDK with Luminox:
...
async function runAgent(conversation: any[]): Promise<[string, any[]]> {
  ...
  while (iteration < maxIterations) {
    ...
    for (const toolCallInfo of toolCallsWithFunction) {
      ...
      // Create tool message for Luminox
      const toolMessage = {
        role: 'tool' as const,
    ...
    // Mark as internal so it won't be sent to Luminox
...
Store all messages (excluding internal ones) to Luminox:
...
<Tip>
**Internal messages**: Mark tool results converted to user messages with `_internal: true` so they aren't sent to Luminox but are still used for the next model iteration.
</Tip>
