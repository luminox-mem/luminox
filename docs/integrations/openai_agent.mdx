...
description: "Integrate OpenAI Agents SDK with Luminox for session persistence, automatic tool execution, and task extraction"
---

The OpenAI Agents SDK provides a high-level framework for building AI agents with automatic tool execution. When integrated with Luminox, you get persistent session management, automatic task extraction, and seamless conversation resumption across sessions.
...
Use `luminox-cli` to quickly set up an OpenAI Agents SDK project with Luminox integration:

```bash
luminox create my-agent-project --template-path "python/openai-agent-basic"
```
...
Install OpenAI Agents SDK and Luminox Python packages:
...
Or with pip:
```bash
pip install openai-agents luminox python-dotenv
```
</Step>
...
Create an OpenAI Agents SDK agent and Luminox client:
...
The OpenAI Agents SDK uses the Responses API format internally, while Luminox uses the Chat Completions API format. The integration handles conversion between these formats automatically.
...
3. **Convert and store**: Convert Responses API format to Chat Completions format and store to Luminox
...
The Agents SDK uses Responses API format (with `function_call` and `function_call_output` items), while Luminox uses Chat Completions API format (with `tool_calls` and `tool` messages). The integration provides conversion utilities:

- **To Luminox**: Use `Converter.items_to_messages()` to convert Responses API format to Chat Completions format
- **From Luminox**: Use `message_to_input_items()` to convert Chat Completions format back to Responses API format

## Basic Integration Pattern

Here's the core pattern for integrating OpenAI Agents SDK with Luminox:
...
# Convert to Chat Completions format and store to Luminox
messages = Converter.items_to_messages(result.to_input_list())
for msg in messages:
    luminox_client.sessions.store_message(
...
Resume conversations by loading previous messages from Luminox and converting them back to Responses API format:

```python
# Load previous conversation
messages = luminox_client.sessions.get_messages(session_id, format="openai")
...
The integration requires converting between Responses API format (used by Agents SDK) and Chat Completions API format (used by Luminox).

### Converting to Luminox Format
...
### Converting from Luminox Format
...
<Tip>
**Format specification**: Always specify `format="openai"` when storing messages to Luminox to ensure proper format handling.
</Tip>
