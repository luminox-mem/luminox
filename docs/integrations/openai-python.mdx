...
description: "Integrate OpenAI Python SDK with Luminox for session persistence, task extraction, and manual tool calling"
---

The OpenAI Python SDK provides direct access to OpenAI's API for building AI applications. When integrated with Luminox, you get persistent session management, automatic task extraction, and full observability of your agent's tool usage and conversations.
...
Use `luminox-cli` to quickly set up an OpenAI Python SDK project with Luminox integration:

```bash
luminox create my-openai-project --template-path "python/openai-basic"
```
...
Install OpenAI and Luminox Python packages:
...
Or with pip:
```bash
pip install openai luminox python-dotenv
```
</Step>
...
The OpenAI Python SDK integration works by storing conversation messages to Luminox in OpenAI message format. Since both use the same format, no conversion is needed.
...
Here's the core pattern for integrating OpenAI Python SDK with Luminox:
...
def append_message(message: dict, conversation: list[dict], session_id: str):
    """Append a message to conversation and store to Luminox."""
...
luminox_client.sessions.store_message(session_id=session_id, blob=message_with_tool_call)
...
luminox_client.sessions.store_message(session_id=session_id, blob=tool_result)
```

## Best Practices

<Tip>
**Message format**: OpenAI message format is compatible with Luminox, so you can store messages directly without conversion. This includes user, assistant, and tool messages. System prompts should be handled through session-level or skill-level configuration rather than as messages.
</Tip>
