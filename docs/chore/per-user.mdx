...
description: "Manage resources per user with Luminox's user-based resource management feature"
---

User-based resource management allows you to associate Luminox resources (Spaces, Sessions, Disks, and Skills) with user identifiers. This enables multi-tenant scenarios, per-user resource isolation, and simplified resource cleanup.
...
- An Luminox API key from [Luminox Dashboard](https://dash.luminox.io), or [local deployment](/settings/local)
...
// Create a disk for the user
const disk = await client.disks.create({ user: 'alice@luminox.io' });
...
disks = client.disks.list(user="alice@luminox.io", limit=20)
print(f"Found {len(disks.items)} disks")

# List skills for a specific user
skills = client.skills.list_catalog(user="alice@luminox.io", limit=20)
...
```python Python
import os
from luminox import LuminoxClient, FileUpload
...
Manage resources for multiple users in a single Luminox project without requiring separate API keys for each user.
</Card>
...
1. Luminox checks if a user with that identifier exists in the project
...
- **Case-sensitive**: `alice@luminox.io` and `alice@luminox.io` are different users
</Accordion>
...
async def main():
    async with AsyncLuminoxClient(
        api_key=os.getenv("LUMINOX_API_KEY"),
    ) as client:
        # All methods are async
        space = await client.spaces.create(user="alice@luminox.io")
        spaces = await client.spaces.list(user="alice@luminox.io")
        await client.users.delete("alice@luminox.io")
...
client.spaces.create(user="alice@luminox.io")
client.sessions.create(user="alice@luminox.io")
...
**Example:**
```bash
curl -X DELETE "https://api.luminox.io/api/v1/user/alice%40example.com" \
