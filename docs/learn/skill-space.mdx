...
Luminox can enable your agent to self-learn skills and standard operating procedures (SOPs) from successful task executions. When you attach a session to a **Space**, all successfully completed tasks are automatically analyzed and converted into reusable knowledge blocks that your agent can reference in future sessions.

## How Skill Learning Works

<Frame caption="Dataflow of Luminox">
<img src="/images/luminox_data_flow.png" alt="Dataflow of Luminox" />
</Frame>


When a session is connected to a Space:

1. **Task Extraction**: As your agent works, Luminox automatically [extracts tasks](/observe/agent_tasks) from the conversation
2. **Task Completion**: When tasks are marked as `success`, they're queued for learning
3. **Automatic Learning**: Luminox analyzes the completed task's execution messages and extracts reusable patterns, approaches, and procedures
...
<Note>
Think of Luminox as giving your AI agent a library that remembers and learns, just like humans do.
</Note>
...
That's because Luminox judges that the New Searching task is too easy, only the user preferences on this task is worthwhile to learn.
...
Luminox analyzes the task execution to extract:
...
Not all completed tasks result in learned skills. After extracting the SOP (Standard Operating Procedure), Luminox evaluates the complexity:
...
Understand how Experience Agent works in Luminox.
</Card>
