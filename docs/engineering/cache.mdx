...
When using [edit strategies](/engineering/editing), the edited prefix of your messages changes with each new message added. This breaks prompt caching because the LLM sees a different prefix every time.
...
client = LuminoxClient(api_key=os.getenv("LUMINOX_API_KEY"))
...
const client = new LuminoxClient({ apiKey: process.env.LUMINOX_API_KEY });
